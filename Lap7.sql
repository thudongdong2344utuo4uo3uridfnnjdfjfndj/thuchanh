...
BEGIN
    ...
    BEGIN
        PRINT 'Tên hãng sản xuất đá tồn tại sản xuất . Vui lòng kiểm tra lại!'
    END
    ELSE
    BEGIN
...
BEGIN
    ...
    END
END
EXEC sp_ThemSuaSanPham 'SP01', 'H01', 'Galaxy Note 11', '50', 'đỏ','1900000.00','Chiếc','Hàng con cao cấp'
...
BEGIN
    ...
    BEGIN
        PRINT 'Mã sản phẩn không tồn tại'
        RETURN
    END
...
BEGIN
    ...
    IF NOT EXISTS (SELECT * FROM Sanpham WHERE masp = @masp)
    BEGIN
        PRINT 'Mã sản phẩn không tồn tại trong bảng  Sanpham.'
        RETURN
    END
    ...
    BEGIN
        PRINT 'số lượng xuất vược quá lượng tồn kho.'
        RETURN
    END
    ...
    BEGIN
        ...
        WHERE sohdx = @sohdx
        PRINT 'cập nhật dữ liệu bản  Xuat thành công.'
    END
    ELSE
    BEGIN
        INSERT INTO Xuat(sohdx, masp, manv, ngayxuat, soluongX)
...
BEGIN
  ...
  BEGIN
    PRINT 'Không tìm thấy sản phẩm đã xóa!'
    RETURN;
  END
